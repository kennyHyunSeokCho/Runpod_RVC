...
from rvc.v2_core.train_losses import discriminator_loss, feature_loss, generator_loss, kl_loss
from rvc.v2_core.vendor.mel_processing import (
    MultiScaleMelSpectrogramLoss,
    mel_spectrogram_torch,
    spec_to_mel_torch,
)
from rvc.v2_core.vendor.utils import (
    HParams,
...
from rvc.v2_core.loop import (
    EpochRecorder,
    check_overtraining,
    update_exponential_moving_average,
    save_to_json,
)
from rvc.v2_core.io import ensure_mute_assets

# Zluda hijack (optional)
try:
	import rvc.lib.zluda
except Exception:
	pass
from rvc.v2_core.vendor.algorithm import commons
...
def main():
    ...
    # 무음 파일 자산 보장
    ensure_mute_assets(os.path.join(current_dir, "logs"), sample_rate=48000, seconds=1.0, min_files=2)
    wavs = glob.glob(
...
def run(
    ...
    # 데이터셋/로더 생성
    from rvc.v2_core.vendor.data_utils import (
    ...
    from rvc.v2_core.vendor.algorithm.discriminators import MultiPeriodDiscriminator
    from rvc.v2_core.vendor.algorithm.synthesizers import Synthesizer
    ...
    if bf16_adamw == True and train_dtype == torch.bfloat16:
        ...
        from rvc.v2_core.vendor.anyprecision_optimizer import AnyPrecisionAdamW
